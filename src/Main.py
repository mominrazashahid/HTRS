...
def main():
    ...
    if args.train or args.validate:
        ...
        DIR = 'segmented'
        words = len([name for name in os.listdir(DIR) if os.path.isfile(os.path.join(DIR, name))])

        for i in range(words):
            infer(model, "segmented/segment" + str(i) + ".png")
            abc = infer.text
            print(abc)
            text_recognized.append(abc)
def grammer():
